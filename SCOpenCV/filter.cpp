...
void filter::get_image(){
	...
	//grayImage = Mat::zeros(480,640,CV_8UC1);
	while(true){
		...
		while(true){
			...
			*/
			for (int i = 0; i < max_rows; i++)
				...
				{   
					frame1[i][j] = gray_in1[i][j].read();
					frame2[i][j] = gray_in2[i][j].read();		      			
				}
	    		}
		...
		while(true){
			...
			*/
			for (int i = 0; i < max_rows; i++)
	    		{
		      		for (int j = 0; j < max_cols; j++) 
					gray_out[i][j].write( results_frame[i][j] );					
	...
	while(true){
		...
		for(int i = 0; i < rows; i++){
			for(int j = 1; j < cols; j++){
				results_frame[i][j] = abs( frame1[i][j] - frame2[i][j] );
